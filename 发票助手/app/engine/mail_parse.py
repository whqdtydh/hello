...
INVOICE_KINDS = [
    ...
    ("餐饮发票", ("餐饮", "饭店", "餐厅", "饮食", "食堂", "酒楼", "饭庄", "小吃", "咖啡", "奶茶", "华铁")),
    ("停车发票", ("停车", "停车场", "泊车")),
    ("加油发票", ("加油", "汽油", "柴油", "石油")),
    ("购物发票", ("超市", "百货", "商城", "便利店", "购物", "商店", "零售", "淘宝", "天猫", "京东", "拼多多")),
...
def kind_from_pdf(text):
    """从发票 PDF 文本识别发票类型（项目名称/票面标识为权威依据）。
    
    该函数在原始 PDF 文本上加入 **完整的文本预处理**（半角化、数字统一、货币统一、日期统一、噪声过滤），
    以提升关键字匹配的鲁棒性。返回值仍为发票种类字符串或空串。
    """
    if not text:
        return ""
    # ---------- 预处理 ----------
    try:
        from .preprocess import preprocess_invoice_text
        pre = preprocess_invoice_text(text)
        cleaned = pre["clean_text"]
    except Exception:
        # 若预处理模块加载失败，回退到原始文本（保持兼容）
        cleaned = text
    # 移除所有空白字符，保持原有的快速匹配逻辑
    t = re.sub(r"\s+", "", cleaned)
    # 按独特标识从强到弱匹配（先精确后宽泛）
    rules = [
        ("机票发票", ("航空运输电子客票行程单", "航空电子客票", "旅客姓名", "航班号", "承运人")),
        ("高铁发票", ("铁路电子客票", "买票请到12306", "电子客票号")),
        ("高速发票", ("通行费", "高速公路通行费", "路桥费", "ETC", "公路车辆通行费")),
        ("电子行程单", ("行程单", "行程码", "行程信息", "行程时间")),
        ("餐饮发票", ("餐饮服务", "美式", "咖啡", "套餐", "餐费", "盒饭")),
        ("酒店发票", ("住宿服务", "酒店", "宾馆", "民宿")),
        ("打车发票", ("客运服务", "滴滴", "网约车", "曹操出行")),
        ("加油发票", ("汽油", "柴油", "加油")),
        ("停车发票", ("停车服务", "停车场", "泊车")),
        ("快递发票", ("快递服务", "物流", "顺丰", "中通")),
        ("通讯发票", ("话费", "电信", "移动", "联通", "宽带")),
