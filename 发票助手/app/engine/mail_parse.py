...
def kind_from_pdf(text):
    ...
    电子发票 PDF 开头通常有明确类型声明或商品项目名称，比邮件主题可靠：
      - 铁路电子客票 PDF：含「铁路电子客票」「买票请到12306」
      - 航空行程单 PDF：含「航空运输电子客票行程单」「旅客姓名」
      - 通行费发票：含「通行费」「ETC」
      - 商品名星号包裹：*生产生活服务*美式 / *住宿服务*标间 / *运输服务*客运服务费
    返回 kind 字符串（如「高铁发票」）；识别不到返回空串（调用方保持原分类）。
    """
    if not text:
        return ""
    t = re.sub(r"\s+", "", text)
